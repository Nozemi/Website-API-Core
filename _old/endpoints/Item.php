...
class Item extends Endpoint {
     ...
     */
    public function get() {
        $item = new \NozCore\Objects\Item();
        if($_REQUEST['endpoint'] == 'items' && isset($_REQUEST['name'])) {
            $this->result = $item->getByName($_REQUEST['name']);
        } else if($_REQUEST['endpoint'] == 'items') {
        ...
        } else if($_REQUEST['endpoint'] == 'item' && isset($_REQUEST['id'])) {
    ...
    public function post() {
        $item = new \NozCore\Objects\Item($GLOBALS['data']);
        $this->result = $item->save();
    }
